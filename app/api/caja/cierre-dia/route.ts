...
type ImageColumn = CompactColumn & { width: number };
...
function wrapText(value: unknown, maxChars: number, maxLines = 2) {
  const words = textoLimpio(value).split(" ").filter(Boolean);
  const lines: string[] = [];
  let current = "";
...
function svgText(params: {
  ...
  const lineHeight = params.lineHeight ?? Math.round(size * 1.24);
  const maxChars = Math.max(4, Math.floor((params.width ?? 280) / (size * 0.54)));
  const lines = wrapText(params.text, maxChars, params.maxLines ?? 1);
  const tspans = lines
    .map((line, index) => {
      const dy = index === 0 ? 0 : lineHeight;
      return `<tspan x="${params.x}" dy="${dy}">${escapeSvg(line)}</tspan>`;
    })
    .join("");
...
function renderSvgMetricCard(params: {
  x: number;
  y: number;
  width: number;
  label: string;
  ...
  const valueSize = params.value.length > 22 ? 24 : params.value.length > 17 ? 27 : 31;

  return `
    <rect x="${params.x}" y="${params.y}" width="${params.width}" height="118" rx="24" fill="#ffffff" stroke="#d9dee8" stroke-width="2"/>
    <rect x="${params.x + 22}" y="${params.y + 28}" width="8" height="62" rx="4" fill="${params.accent}"/>
    ${svgText({
      x: params.x + 46,
      y: params.y + 42,
...
function buildImageColumns(financieras: string[]): ImageColumn[] {
  return [
    { key: "venta", title: "# VENTA", width: 150 },
    { key: "servicio", title: "SERVICIO", width: 170 },
    { key: "imei", title: "IMEI", width: 220 },
    { key: "jalador", title: "JALADOR", width: 250 },
    { key: "ingresos", title: "INGRESOS", width: 180, align: "right", tone: "money" },
    { key: "detalleIngresos", title: "DETALLES DE INGRESO", width: 360 },
    ...financieras.map((nombre) => ({
      key: `fin_${nombre}`,
      title: nombre,
      width: 190,
    ...
    { key: "utilidad", title: "UTILIDAD", width: 180, align: "right", tone: "money" },
    { key: "vendedor", title: "VENDEDOR", width: 200 },
    { key: "comision", title: "COMISION", width: 170, align: "right", tone: "danger" },
    { key: "salida", title: "SALIDA", width: 170, align: "right", tone: "danger" },
    { key: "caja", title: "CAJA", width: 180, align: "right", tone: "money" },
  ];
}

function renderSvgTableHeader(columns: ImageColumn[], x: number, y: number) {
  let cursor = x;
        ...
        column.tone === "financial"
          ...
          : column.tone === "danger"
            ...
            : column.tone === "money"
              ...
              : "#151923";
      const textX =
        column.align === "right" ? cursor + column.width - 18 : cursor + 18;
      const anchor = column.align === "right" ? "end" : "start";
      const cell = `
        <rect x="${cursor}" y="${y}" width="${column.width}" height="64" fill="${fill}" stroke="#404a5f" stroke-width="1"/>
        ${svgText({
          x: textX,
          y: y + 30,
          text: column.title,
          fill: "#ffffff",
          size: column.tone === "financial" ? 15 : 16,
          weight: 800,
...
function renderSvgTableRow(
  ...
  return columns
    .map((column) => {
      ...
      const anchor = column.align === "right" ? "end" : "start";
      const cell = `
        <rect x="${cursor}" y="${y}" width="${column.width}" height="62" fill="${fill}" stroke="#e2e5ea" stroke-width="1"/>
        ${svgText({
          x: textX,
          y: y + 27,
          text,
          fill: color,
          size: 15,
          weight: column.tone ? 700 : 500,
          width: column.width - 30,
          anchor,
          maxLines: column.key === "detalleIngresos" ? 2 : 1,
        })}
      `;
      cursor += column.width;
      return cell;
...
function renderSvgCashFlow(params: {
  x: number;
  y: number;
  width: number;
  totals: ReturnType<typeof buildTrialTotals>;
  movimientos: CashMovementRow[];
}) {
  const summaryGap = 24;
  const summaryWidth = (params.width - 48 - summaryGap * 3) / 4;
  const cards = [
    { label: "Ingresos caja", value: formatoPesos(params.totals.ingresosCaja), color: "#1f6b4f" },
    { label: "Egresos caja", value: formatoPesos(params.totals.egresosCaja), color: "#9f2737" },
    {
      label: "Neto caja dia",
      value: formatoPesos(params.totals.cajaNeta),
      color: params.totals.cajaNeta < 0 ? "#9f2737" : "#1f6b4f",
    },
    { label: "Movimientos", value: String(params.movimientos.length), color: "#2f4d75" },
  ];
  const cashColumns: ImageColumn[] = [
    { key: "tipo", title: "MOVIMIENTO", width: 230 },
    { key: "concepto", title: "CONCEPTO", width: params.width - 760 },
    { key: "sede", title: "SEDE", width: 250 },
    { key: "valor", title: "VALOR", width: 250, align: "right", tone: "money" },
  ];
  let y = params.y;

  let svg = `
    <rect x="${params.x}" y="${y}" width="${params.width}" height="190" rx="30" fill="#ffffff" stroke="#d9dee8" stroke-width="2"/>
    ${svgText({ x: params.x + 32, y: y + 46, text: "FLUJO DE CAJA DEL DIA", fill: "#151923", size: 26, weight: 900, width: 520 })}
    ${svgText({ x: params.x + 32, y: y + 82, text: "Movimientos manuales y cartera incluidos en el cierre.", fill: "#667085", size: 17, weight: 500, width: 700 })}
  `;

  cards.forEach((card, index) => {
    svg += renderSvgMetricCard({
      x: params.x + 32 + index * (summaryWidth + summaryGap),
      y: y + 118,
      width: summaryWidth,
      label: card.label,
      value: card.value,
  ...
  y += 26;
  svg += renderSvgTableHeader(cashColumns, params.x, y);
  y += 64;

  if (!params.movimientos.length) {
    svg += `
      <rect x="${params.x}" y="${y}" width="${params.width}" height="62" fill="#ffffff" stroke="#e2e5ea" stroke-width="1"/>
      ${svgText({ x: params.x + 18, y: y + 36, text: "No hay ingresos o egresos registrados para este dia.", fill: "#667085", size: 18, weight: 600, width: params.width - 36 })}
    `;
    return { svg, height: y + 62 - params.y };
  }

  params.movimientos.forEach((movimiento, index) => {
    svg += renderSvgTableRow(
      cashColumns,
      {
  ...
  return { svg, height: y - params.y };
}

async function buildPngCierreTabla(params: {
  periodoLabel: string;
  cobertura: string;
  rows: SaleTableRow[];
  financieras: string[];
  ...
  const width = Math.max(2600, tableWidth + 160);
  const x = 80;
  const contentWidth = width - 160;
  const rowCount = Math.max(params.rows.length, 1);
  const cashHeight =
    270 + 26 + 64 + Math.max(params.movimientos.length, 1) * 62;
  const height = 760 + 64 + rowCount * 62 + 90 + cashHeight + 80;

  const cardGap = 28;
  const cardWidth = (contentWidth - cardGap * 3) / 4;
  const metricCards = [
    { label: "Ingresos del dia", value: formatoPesos(totals.ingresosAcumulados), color: "#1f6b4f" },
    { label: "Ingreso ventas", value: formatoPesos(totals.ingresosVentas), color: "#1f6b4f" },
    { label: "Ingreso caja", value: formatoPesos(totals.ingresosCaja), color: "#2d6f82" },
    { label: "Transferencia", value: formatoPesos(totals.transferencia), color: "#344f7a" },
    { label: "Voucher", value: formatoPesos(totals.voucher), color: "#7652a6" },
    { label: "Financieras", value: formatoPesos(totals.financieras), color: "#2f4d75" },
    { label: "Utilidad", value: formatoPesos(totals.utilidad), color: "#9a7324" },
    { label: "Caja neta dia", value: formatoPesos(totals.cajaNeta), color: totals.cajaNeta < 0 ? "#9f2737" : "#1f6b4f" },
  ];
  const maxSales = Math.max(totals.ventas, params.previousSalesCount, 1);
  const currentBar = totals.ventas > 0 ? Math.max(18, (contentWidth - 360) * totals.ventas / maxSales) : 0;
  const previousBar = params.previousSalesCount > 0 ? Math.max(18, (contentWidth - 360) * params.previousSalesCount / maxSales) : 0;
  const diff = totals.ventas - params.previousSalesCount;
  const diffText =
    params.previousSalesCount === 0
      ? totals.ventas > 0
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      <defs>
        <linearGradient id="hero" x1="0" y1="0" x2="1" y2="1">
          ...
          <stop offset="1" stop-color="#812630"/>
        </linearGradient>
      </defs>
      <rect width="${width}" height="${height}" fill="#f3f5f8"/>
      <rect x="${x}" y="56" width="${contentWidth}" height="190" rx="36" fill="url(#hero)"/>
      <rect x="${x + 34}" y="86" width="128" height="128" rx="28" fill="#ffffff"/>
      ${
        logoDataUrl
          ? `<image href="${logoDataUrl}" x="${x + 44}" y="96" width="108" height="108" preserveAspectRatio="xMidYMid meet"/>`
          : svgText({ x: x + 88, y: 160, text: "C", fill: "#ef3333", size: 54, weight: 900, width: 80 })
      }
      ${svgText({ x: x + 190, y: 124, text: "CIERRE DEL DIA", fill: "#ffffff", size: 54, weight: 900, width: 680 })}
      ${svgText({ x: x + 194, y: 170, text: "CONECTAMOS FINAN SERVICES S.A.S", fill: "#f8fafc", size: 21, weight: 800, width: 600, letterSpacing: 1.5 })}
      ${svgText({ x: x + 194, y: 205, text: `${params.periodoLabel} | ${params.cobertura}`, fill: "#cbd5e1", size: 20, weight: 500, width: 820 })}
      <rect x="${width - 620}" y="96" width="460" height="92" rx="24" fill="#ffffff" stroke="#d9dee8" stroke-width="2"/>
      ${svgText({ x: width - 590, y: 132, text: "CAJA ACUMULADA", fill: "#667085", size: 17, weight: 800, width: 390, letterSpacing: 1.2 })}
      ${svgText({ x: width - 590, y: 172, text: formatoPesos(params.cajaAcumulada), fill: params.cajaAcumulada < 0 ? "#9f2737" : "#1f6b4f", size: 31, weight: 900, width: 390 })}
  `;
  ...
  svg += `
    <rect x="${x}" y="606" width="${contentWidth}" height="120" rx="28" fill="#ffffff" stroke="#d9dee8" stroke-width="2"/>
    ${svgText({ x: x + 34, y: 652, text: "COMPARATIVO DE VENTAS", fill: "#151923", size: 25, weight: 900, width: 520 })}
    ${svgText({ x: width - 560, y: 650, text: diffText, fill: diff >= 0 ? "#1f6b4f" : "#9f2737", size: 21, weight: 800, width: 400, anchor: "end" })}
    ${svgText({ x: x + 34, y: 688, text: "Hoy", fill: "#667085", size: 18, weight: 800, width: 110 })}
    <rect x="${x + 150}" y="673" width="${contentWidth - 360}" height="18" rx="9" fill="#edf0f5"/>
    ${currentBar > 0 ? `<rect x="${x + 150}" y="673" width="${currentBar}" height="18" rx="9" fill="#1f6b4f"/>` : ""}
    ${svgText({ x: width - 180, y: 690, text: String(totals.ventas), fill: "#151923", size: 20, weight: 900, width: 90, anchor: "end" })}
    ${svgText({ x: x + 34, y: 716, text: "Ayer", fill: "#667085", size: 18, weight: 800, width: 110 })}
    <rect x="${x + 150}" y="701" width="${contentWidth - 360}" height="18" rx="9" fill="#edf0f5"/>
    ${previousBar > 0 ? `<rect x="${x + 150}" y="701" width="${previousBar}" height="18" rx="9" fill="#7a8799"/>` : ""}
    ${svgText({ x: width - 180, y: 718, text: String(params.previousSalesCount), fill: "#151923", size: 20, weight: 900, width: 90, anchor: "end" })}
    ${svgText({ x, y: 785, text: "VENTAS DEL DIA", fill: "#151923", size: 28, weight: 900, width: 460 })}
  `;

  let y = 820;
  svg += renderSvgTableHeader(columns, x, y);
  y += 64;

  if (!params.rows.length) {
    svg += `
      <rect x="${x}" y="${y}" width="${tableWidth}" height="62" fill="#ffffff" stroke="#e2e5ea" stroke-width="1"/>
      ${svgText({ x: x + 18, y: y + 38, text: "No hay ventas registradas para este dia.", fill: "#667085", size: 18, weight: 600, width: tableWidth - 36 })}
    `;
    y += 62;
  } else {
    params.rows.forEach((row, index) => {
      ...
      svg += renderSvgTableRow(columns, values, x, y, index);
      y += 62;
    });
  }

  y += 80;
  const cash = renderSvgCashFlow({
    x,
    y,
    width: contentWidth,
    totals,
...
export async function GET(req: Request) {
  try {
    ...
    if (vista === "tabla") {
      ...
      if (formato === "png" || formato === "imagen" || formato === "image") {
        const pngBuffer = await buildPngCierreTabla({
        ...
        return new Response(Uint8Array.from(pngBuffer), {
          ...
          headers: {
            ...
            "Content-Disposition": `inline; filename="${baseFileName}.png"`,
            "Cache-Control": "no-store",
          },
        });
      }
