...
export default async function DashboardPage() {
  ...
  const [resumenComercialMensual, resumenInventarioAdmin] = await Promise.all([
    esVendedor || esFacturador
      ? Promise.resolve(null)
      : getMonthlyCommercialSummary({
          sedeId: esAdmin ? null : session.sedeId ?? null,
        }),
    esAdmin ? getAdminInventorySummary() : Promise.resolve(null),
  ]);
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f5f2ea_0%,#eef3f9_100%)] text-slate-950">
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {esAdmin && resumenInventarioAdmin && (
          <AdminInventoryRadarSection summary={resumenInventarioAdmin} />
        )}
